...
class S3Client extends AwsClient implements S3ClientInterface {
    ...
    public function __construct(array $args) {
        ...
        $stack->appendBuild(
                Middleware::contentType(['PutObject', 'UploadPart']), 's3.content_type'
        );
    ...
    public function createPresignedRequest(CommandInterface $command, $expires) {
        ...
        $signer = call_user_func(
                $this->getSignatureProvider(), $this->getConfig('signature_version'), $this->getConfig('signing_name'), $this->getConfig('signing_region')
        );

        return $signer->presign(
                        \Aws\serialize($command), $this->getCredentials()->wait(), $expires
        );
    }
    ...
    public static function _applyRetryConfig($value, $_, HandlerList $list) {
        ...
        $decider = function ($retries, $command, $request, $result, $error) use ($decider, $value) {
            ...
            if ($decider($retries, $command, $request, $result, $error)) {
                ...
                if (
                        $error->getResponse() && $error->getResponse()->getStatusCode() >= 400
                ) {
                    return strpos(
                                    $error->getResponse()->getBody(), 'Your socket connection to the server'
                            ) !== false;
    ...
    public static function _applyApiProvider($value, array &$args, HandlerList $list) {
        ...
        $args['parser'] = new GetBucketLocationParser(
                ...
                $args['parser'], $args['exception_class']
                ), $args['error_parser'], $args['exception_class']
                )
        );
    }
