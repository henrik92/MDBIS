...
final class Middleware {
    ...
    Service $api, $bodyParameter = 'Body', $sourceParameter = 'SourceFile'
    ) {
    ...
    public static function validation(Service $api, Validator $validator = null) {
        ...
        return function (callable $handler) use ($api, $validator) {
            return function (
                ...
                $operation = $api->getOperation($command->getName());
                $validator->validate(
                        $command->getName(), $operation->getInput(), $command->toArray()
                );
                return $handler($command, $request);
            };
        };
        ...
        return function (callable $handler) use ($signatureFunction, $credProvider) {
            return function (
                ...
                return $credProvider()->then(
                            ...
                            return $handler(
                                    $command, $signer->signRequest($request, $creds)
                            );
                        }
                );
            };
    ...
    callable $decider = null, callable $delay = null, $stats = false
    ) {
        ...
        return function (callable $handler) {
            return function (
                ...
                return $handler($command, $request->withHeader(
                                'aws-sdk-invocation-id', md5(uniqid(gethostname(), true))
                ));
            };
        };
    }
            ...
            return function (
                ...
                if (!$request->hasHeader('Content-Type') && in_array($command->getName(), $operations, true) && ($uri = $request->getBody()->getMetadata('uri'))
                    ...
                    $request = $request->withHeader(
                            'Content-Type', Psr7\mimetype_from_filename($uri) ?: 'application/octet-stream'
                    );
                }
